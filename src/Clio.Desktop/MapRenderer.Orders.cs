...
namespace Clio.Desktop
{
    internal sealed partial class MapRenderer
    {
        private void DrawOrderPreview(Graphics g, Game game)
        {
            ...
            Color ink = EncounterRules.HostileAt(game, destination, actor.Id) ? Color.FromArgb(231, 171, 109) : Color.FromArgb(185, 223, 154);
            using (Brush light = new SolidBrush(Color.FromArgb(25, ink))) g.FillPolygon(light, target.Polygon);
            using (Pen glow = new Pen(Color.FromArgb(46, ink), 6)) g.DrawPolygon(glow, target.Polygon);
            ...
            using (Pen under = new Pen(Color.FromArgb(140, 18, 37, 36), 4.5f)) g.DrawLine(under, from, to);
            ...
            if (game.TerrainTravelEnabled && Zoom >= 2)
            {
                ...
                using (Brush shade = new SolidBrush(Color.FromArgb(225, 23, 43, 44))) g.FillEllipse(shade, badge);
