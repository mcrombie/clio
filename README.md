...
The current source corresponds to **livestock-29**. It uses a native Windows Forms interface and an engine-independent simulation. A separate Unity presentation starter is included for future development.
...
The executable and simulation library are written to `build/livestock-29/`. Keep them together. Build output and saved games are not included in the repository.
