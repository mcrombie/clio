...
Stories are now saved as `CLIO-STORY-16`. The named `key=value` format introduced in version 15 retains the required `seed`, `style`, `ancestry`, `bands`, `name`, `culture`, `pace`, `rules`, `places`, `salt`, `tribes`, `terrain`, `personalities`, `gatherings` and `decisions` keys, followed by a blank line and recorded commands. Version 16 adds `guided=GuidedOpening` for the experimental influence opening, or `guided=LegacyOpening` for existing modes. This new key is optional when reading version 16; absence keeps the old rules. Unknown keys, repeated keys and unknown flag values are rejected. Versions 1–14 retain their frozen positional readers; version 15 retains its original named reader and never enters the new opening. All new writes use version 16, which older executables cannot read.

The guided opening is a founding simulation setting rather than presentation metadata. Its influence balance, turn and regional resource changes are rebuilt through the normal command replay, using `end`, `guided-gather` and `guided-move:<cellId>`. The destination cell anchors a region in the seeded world. No `enable-guided` upgrade is applied to old saves. The existing decision metadata schema remains unchanged. See [the guided opening](GUIDED_OPENING.md).
